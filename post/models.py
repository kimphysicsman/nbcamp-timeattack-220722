...
class JobPostActivityStatus(models.Model):
    status = models.CharField("지원서 상태", max_length=100)
...
class JobPostActivity(models.Model):
    ...
    status = models.ForeignKey(JobPostActivityStatus, on_delete=models.SET_NULL, null=True)
...
class JobPostStatus(models.Model):
    status = models.CharField("채용진행상황", max_length=100)
...
class JobPostStatusLog(models.Model):
    apply = models.ForeignKey(JobPostActivity, verbose_name="지원서", on_delete=models.CASCADE)
    status = models.ForeignKey(JobPostStatus, verbose_name="채용진행상황", on_delete=models.CASCADE)

    class Meta:
        db_table = "job_post_status_log"

    def __str__(self):
        return f"{self.apply.user.username}'s apply - {self.status.status}"
