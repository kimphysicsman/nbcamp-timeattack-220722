...
class ApplyView(APIView):
    ...
    def get(self, request):
        ...
        try:
            status = JobPostActivityStatus.objects.get(status=status)
        except:
            status = None
        apply_status = JobPostActivity.objects.filter(status=status)
        return Response(JobPostActivitySerializer(apply_status, many=True).data)


    def post(self, request):
        request.data['user']= request.user.id
        try:
            status_init = JobPostActivityStatus.objects.get(status="submitted")
        except:
            status_init = JobPostActivityStatus.objects.create(status="submitted")
        
        apply_serialzer = JobPostActivitySerializer(data=request.data)
        if apply_serialzer.is_valid():
            apply_serialzer.save(status=status_init)
            return Response(status=status.HTTP_200_OK)
